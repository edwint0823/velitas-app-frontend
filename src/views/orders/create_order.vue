...
const schema = yup.object({
  customer_email: yup.string().default('').email(createOrderValidation.validEmail).required(createOrderValidation.requiredEmail),
  customer_name: yup.string().default('').required(createOrderValidation.requiredName).max(50),
  customer_tel: yup.string().required(createOrderValidation.requiredPhone).max(12, createOrderValidation.completePhone).min(12, createOrderValidation.completePhone)
})
